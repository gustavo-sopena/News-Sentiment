...
def main():
    ...
    parser.add_argument("--category", required=True, help="News category to fetch")
    parser.add_argument("--sample", type=int, default=50, help="Number of headlines to analyze")
    ...
    headlines = fetch_headlines(args.category, args.sample, api_key=api_key)
    counts = {"Positive": 0, "Neutral": 0, "Negative": 0, "Unknown": 0}
    for h in headlines:
        sentiment, score = score_headline(h.get("title", ""))
        counts[sentiment] = counts.get(sentiment, 0) + 1

    # Plot and optional CSV
    plot_counts(counts, args.out, title=f"Sentiment for {args.category}")
    if args.csv:
        write_csv_counts(counts, args.csv, category=args.category, sample_size=len(headlines))
